...
export class DatabaseHealthResponseDto {
  ...
  @ApiProperty({ example: 'Database connection successful', description: 'Mensagem de status' })
  message: string;
  
  @ApiProperty({ example: '2025-10-03T14:56:10.123Z', description: 'Timestamp da resposta' })
  timestamp: string;
...
export class AppController {
  constructor(private readonly prisma: PrismaService) {}
    ...
    @ApiOperation({ 
      ...
      description: 'Endpoint para verificar se a conexão com o banco de dados PostgreSQL está funcionando'
    })
    @ApiResponse({
      status: 200,
